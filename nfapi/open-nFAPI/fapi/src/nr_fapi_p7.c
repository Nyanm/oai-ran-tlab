...
static uint8_t pack_dl_tti_pdcch_pdu_rel15_value(void *tlv, uint8_t **ppWritePackedMsg, uint8_t *end)
{
  ...
  for (uint_fast16_t i = 0; i < p->numSpatialStreams; i++)
    if (!(push16(p->dci_spatialSteamMap[i].dci_index, ppWritePackedMsg, end)
          && push16(p->dci_spatialSteamMap[i].spatial_steam_index, ppWritePackedMsg, end)))
      return 0;
#endif
  return 1;
}
...
static uint8_t pack_dl_tti_pdsch_pdu_rel15_value(void *tlv, uint8_t **ppWritePackedMsg, uint8_t *end)
{
  ...
  for (uint_fast8_t c = 0; c < value->param_v4.numberCodewords; c++)
    if (!pack_spatial_stream_indices(value->param_v4.spatialSteamsCw + c, ppWritePackedMsg, end))
      return 0;
#endif
  return 1;
}
...
static uint8_t unpack_dl_tti_pdcch_pdu_rel15_value(void *tlv, uint8_t **ppReadPackedMsg, uint8_t *end)
{
  ...
  for (uint_fast16_t i = 0; i < p->numSpatialStreams; i++) {
    if (!(pull16(ppReadPackedMsg, &p->dci_spatialSteamMap[i].dci_index, end)
          && pull16(ppReadPackedMsg, &p->dci_spatialSteamMap[i].spatial_steam_index, end)))
      return 0;
  }
#endif
  return 1;
...
static uint8_t unpack_dl_tti_pdsch_pdu_rel15_value(void *tlv, uint8_t **ppReadPackedMsg, uint8_t *end)
{
  ...
  for (uint_fast8_t c = 0; c < value->param_v4.numberCodewords; c++)
    if (!unpack_spatial_stream_indices(value->param_v4.spatialSteamsCw + c, ppReadPackedMsg, end))
      return 0;
#endif
  return 1;
}
