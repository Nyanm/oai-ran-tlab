...
// the number of streams could be same as number of number of layers or number
// of antenna ports or number of baseband ports. Hence we set this to be the
// maximum number of baseband ports
#define MAX_NUM_SPATIAL_STREAMS 16

typedef struct {
  uint16_t dci_index;
  uint16_t spatial_steam_index;
} nfapi_v4_dci_spatial_steam_index_t;

typedef struct {
  uint16_t numSpatialStreams;
  nfapi_v4_dci_spatial_steam_index_t dci_spatialSteamMap[MAX_NUM_SPATIAL_STREAMS];
...
typedef struct {
  ...
  nfapi_nr_spatial_stream_index_t spatialSteamsCw[MAX_NUM_CODEWORDS];
