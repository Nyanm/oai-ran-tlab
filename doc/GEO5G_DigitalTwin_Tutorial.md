...
|-----------|-----------|--------|
| OAI 5G RAN (gNB + UE) | `openairinterface5g` | `origin/taps-client-rewrite-fixed` |
| Ray-Tracing Emulator | `raytracing-channel-emulator` | `origin/eurecom_simulation_godot_integration` |
| OAI LMF | `oai-cn5g-lmf` | `origin/fix-unit-mismatch` |
...
cd oai-cn5g-lmf
git checkout origin/fix-unit-mismatch
```
...
Follow the setup instructions in the emulator's own README:

```
raytracing-channel-emulator/server/README.md
```

Install Python dependencies and verify the EURECOM scene configuration is present at:

```
raytracing-channel-emulator/server/scenes/EURECOM/example_config.yaml
```
...
git checkout origin/taps-client-rewrite-fixed
...
docker build \
  ...
  -t oai-lmf:fix-unit-mismatch .
```

#### Configure the compose file to use the custom LMF

Open the positioning compose file:

```bash
cd ~/openairinterface5g/doc/tutorial_resources/oai-cn5g
nano docker-compose-positioning.yaml
```

Find the LMF service definition and update the image tag:

```yaml
# Change:
#   image: oai-lmf:latest
# To:
    image: oai-lmf:fix-unit-mismatch
```
...
sudo ./nr-softmodem \
  -O ../../../targets/PROJECTS/GENERIC-NR-5GC/CONF/gnb.sa.band78.fr1.106PRB.usrpb2101.conf \
...
### 7.2 Send a Positioning Request and Start the Emulator
...
# 4. Stop CN5G
cd ~/openairinterface5g/doc/tutorial_resources/oai-cn5g
docker-compose -f docker-compose-positioning.yaml down
```
...
```bash
docker-compose -f docker-compose-positioning.yaml down -v
```

Verify the LMF image was built successfully:

```bash
docker images | grep oai-lmf
```
...
Ensure `InputData.json` conforms to 3GPP TS 29.572.

**gNB or UE exits immediately**

Confirm the correct branch (`origin/taps-client-rewrite-fixed`) is checked out and that the build included `-DOAI_VRTSIM_TAPS_CLIENT=ON`. Check that the IPC socket paths are not already in use.
