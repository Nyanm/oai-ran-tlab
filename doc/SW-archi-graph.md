...
- call the scheduler through `NR_ul_indication()`, which corresponds to both an
  `RX_data.indication` and `CRC.indication` in FAPI parlance
...
`L1_tx_thread()` (`L1_rx_thread()`), both TX and RX processing runs in
parallel.
