...
#include "l2-fapi-proxy/src/queue.h"  // L2 proxy: For queue operations
#include "NR_UE_PHY_INTERFACE/NR_IF_Module.h"  // For nr_ue_if_module_init
...
// Forward declaration for L2 proxy PHY stub task
static void *NRUE_phy_stub_standalone_pnf_task(void *arg);

void init_nrUE_standalone_thread(int ue_idx)
{
  LOG_I(NR_MAC, "[UE %d] Initializing nFAPI PNF standalone mode\n", ue_idx);
  int standalone_tx_port = 3611 + ue_idx * 2;
  int standalone_rx_port = 3612 + ue_idx * 2;
  LOG_I(NR_MAC, "[UE %d] Setting up standalone sockets: TX port=%d, RX port=%d\n", 
        ue_idx, standalone_tx_port, standalone_rx_port);
  nrue_init_standalone_socket(standalone_tx_port, standalone_rx_port);

  NR_UE_MAC_INST_t *mac = get_mac_inst(0);
  pthread_mutex_init(&mac->mutex_dl_info, NULL);

  // Initialize IF module for emulated L1 mode
  mac->if_module = nr_ue_if_module_init(0);
  AssertFatal(mac->if_module, "can not initialize IF module\n");
  LOG_I(NR_MAC, "[UE %d] IF module initialized for standalone PNF mode\n", ue_idx);

  LOG_I(NR_MAC, "[UE %d] Creating nFAPI PNF task thread\n", ue_idx);
  pthread_t thread;
  ...
  LOG_I(NR_MAC, "[UE %d] Creating PHY stub standalone PNF task thread\n", ue_idx);
  ...
  LOG_I(NR_MAC, "[UE %d] nFAPI PNF standalone threads created successfully\n", ue_idx);
}
...
static void process_queued_nr_nfapi_msgs(NR_UE_MAC_INST_t *mac, int sfn, int slot)
{
  ...
  // if (ul_tti_request_crc_curr_slot) {
  //   LOG_I(NR_MAC, "[DEQUEUE] Found and dequeued UL_TTI_REQUEST for current slot %d.%d\n", sfn, slot);
  // }
  ...
  for (int i = 0; i < NR_MAX_HARQ_PROCESSES; i++) {
    ...
    if (ul_tti_request_crc && ul_tti_request_crc->n_pdus > 0) {
      // LOG_I(NR_MAC, "[DEQUEUE] Processing ul_tti_req for sfn/slot %d.%d with %d PDUs\n", sfn, slot, ul_tti_request_crc->n_pdus);
  ...
  if (rach_ind && rach_ind->number_of_pdus > 0) {
      LOG_I(NR_MAC, "[UE] Sending RACH_IND to Proxy for sfn/slot %d.%d with %d PDUs\n", 
  ...
  // NOTE: RX_IND/CRC_IND are processed in send_crc_ind_and_rx_ind() 
  // which is called from copy_ul_tti_data_req_to_dl_info() during DL indication processing.
  // Do NOT dequeue them here to avoid double-free!

  if (dl_tti_request) {
    // LOG_I(NR_MAC, "[DEQUEUE] Found DL_TTI_REQUEST for sfn/slot %d.%d with %d PDUs\n",
    ...
    if (!tx_data_request) {
      ...
      LOG_W(NR_MAC, "[%d.%d] Skipping processing: dl_tti has %d PDUs, tx_req has %d PDUs\n",
            dl_tti_request->SFN, dl_tti_request->Slot,
...
static void *NRUE_phy_stub_standalone_pnf_task(void *arg)
{
  ...
  while (!oai_exit) {
    ...
    // Always process queued messages, not just when waiting for MIB
    process_queued_nr_nfapi_msgs(mac, frame, slot);
    if (IS_SA_MODE(get_softmodem_params()) && mac->mib == NULL) {
      // Verbose MIB wait log commented out - MIB reception working correctly
      // LOG_I(NR_MAC, "Waiting for MIB at frame %d.%d, mac->state=%d\n", frame, slot, mac->state);
