                                       ...
                                       int current_harq_pid)
{
  ...
  * this helps allocate retransmission when TDA changes (e.g. new nrOfSymbols > old nrOfSymbols) */
  ...
  new_sched.ant_port_idx.numSpatialStreamIndices = p->XP * p->N1 * p->N2;
  get_antenna_port_indices(beam_idx,
                                                     ...
                                                     int pdu_index)
{
  ...
  fill_dig_bf_interface_list(beam_index,
                             pdsch_pdu->param_v4.spatialStreamsCw[0].numSpatialStreamIndices,
                             0,
                             pdsch_pdu->precodingAndBeamforming.prgs_list[0].dig_bf_interface_list);
  return pdsch_pdu;
}
