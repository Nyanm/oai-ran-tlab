...
void print_period_offset_beam()
{
  for (int i = 0; i < MAX_MOBILES_PER_GNB * SLOT_TYPE_NUM; i++) {
    printf("p %d %d %d\n", i , reserve_period[i], reserve_offset[i]);
  }
  fflush(stdout);
}
...
int get_first_ul_slot_beam(const frame_structure_t *fs, int beam_idx, int beams_per_period, int num_beam, int uid)
{
  ...
  LOG_I(NR_MAC, "get_first_ul_slot_beam 0 idx %d beam_idx %d num_beam %d\n", idx, beam_idx, num_beam);
...
int get_ul_slot_offset_beam(const frame_structure_t *fs, int idx, bool is_csi, int beam_idx, int beams_per_period, int num_beam, bool set)
{
  ...
  LOG_I(NR_MAC, "get_ul_slot_offset_beam 0 idx %d is_csi %d beam_idx %d num_beam %d\n", idx, is_csi, beam_idx, num_beam);

  int uid = (is_csi) ? idx/2 : idx;
  int id = uid / beams_per_period;
  //printf("get_ul_slot_offset_beam 1 id %d idx %d is_csi %d beam_idx %d num_beam %d\n", id, idx, is_csi, beam_idx, num_beam);
  ...
  idx = 3 * id + type;
  LOG_I(NR_MAC, "get_ul_slot_offset_beam 2 uid %d type %d id %d idx %d is_csi %d beam_idx %d num_beam %d\n", uid, type, id, idx, is_csi, beam_idx, num_beam);
...
void get_ul_slot_period_beam(const frame_structure_t *fs, int idx, bool is_csi, int beam_idx, int period, bool set)
{
  ...
  LOG_I(NR_MAC, "get_ul_slot_period_beam 0 idx %d is_csi %d beam_idx %d period %d\n", idx, is_csi, beam_idx, period);

  int uid = (is_csi) ? idx/2 : idx;
  //printf("get_ul_slot_period_beam 1 uid %d idx %d is_csi %d beam_idx %d\n", uid, idx, is_csi, beam_idx);
...
static void config_sched_ctrlCommon(gNB_MAC_INST *nr_mac)
{
  ...
  NR_sched_ctrlCommon_t *sched_ctrlCommon = calloc_or_fail(1, sizeof(*sched_ctrlCommon));
  ...
  if (type0_PDCCH_CSS_config.type0_pdcch_ss_mux_pattern > 1) {
    AssertFatal (nr_mac->cset0_bwp_start % 6 == 0,
                 "commonControlResourceSet 1st RB needs to be multiple of 6 %d and needs to be contained in the bandwidth of CSET0\n", nr_mac->cset0_bwp_start);
