...
void nr_preprocessor_phytest(gNB_MAC_INST *mac, post_process_pdsch_t *pp_pdsch)
{
  ...
  const bool is_mu_mimo = get_softmodem_params()->mu_mimo;
  ...
  UE_iterator (mac->UE_info.connected_ue_list, UE) {
    ...
    if (!is_mu_mimo) {
      ...
      beam = beam_allocation_procedure(&mac->beam_info, frame, slot, beam_idx, mac->frame_structure.numb_slots_frame);
      ...
      // Keep the pre-assigned beams in create_new_UE() for MU-MIMO.
      beam = beam_allocation_procedure(&mac->beam_info, frame, slot, UE->UE_beam_index, mac->frame_structure.numb_slots_frame);
    }
    ...
    // Modify DMRS ports for MU-MIMO. TODO: modify this in get_dl_dmrs_params().
    if (is_mu_mimo) {
      // Assumes only two UEs, one per group.
      AssertFatal(sched_pdsch.nrOfLayers == 2, "MU-MIMO phytest supports only 2 layers per UE\n");
      if (beam.idx == 0)
        sched_pdsch.dmrs_parms.dmrs_ports_id = 2; // ports 0,1
      else if (beam.idx == 1)
        sched_pdsch.dmrs_parms.dmrs_ports_id = 8; // ports 2,3
      else
        DevAssert(0);
      sched_pdsch.dmrs_parms.numDmrsCdmGrpsNoData = 2;
    }
...
void nr_ul_preprocessor_phytest(gNB_MAC_INST *nr_mac, post_process_pusch_t *pp_pusch)
{
  ...
  UE_iterator (nr_mac->UE_info.connected_ue_list, UE) {
    ...
    int beam = ++last_ue_beam;
    AssertFatal(beam < 2, "No more than 2 concurrent beams supported in phytest\n");
    ...
    /* mark the corresponding RBs as used */
    fill_pdcch_vrb_map(nr_mac, CC_id, &sched_ctrl->sched_pdcch, CCEIndex, sched_ctrl->aggregation_level, beam);
