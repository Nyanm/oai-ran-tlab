...
retry:
    ...
    //fixme:
    //if(device->openair0_cfg->nr_band >= 257)
      //  *(uint64_t *)ptimestamp -= device->openair0_cfg->tx_sample_advance;
...
void trx_oxgrf_end(openair0_device *device)
{
    ...
    for(int i = 0; i < oxgrf->tx_num_channels; i++) {
        uint32_t count = 0;
        oxgrf_get_channel_event(oxgrf->dev, OXGRF_TX_CHANNEL_TIMEOUT, i+1, &count);
        LOG_I(HW, "[oxgrf] TX%d Channel timeout: %u\n", i+1, count);
    }
    for(int i = 0; i < oxgrf->rx_num_channels; i++) {
        uint32_t count = 0;
        oxgrf_get_channel_event(oxgrf->dev, OXGRF_RX_CHANNEL_OVERFLOW, i+1, &count);
...
int device_init(openair0_device *device, openair0_config_t *openair0_cfg)
{
    ...
    if(model == 550) {
        ...
        oxgrf->BoardType = IQX8400;
        shift = 2;
        LOG_I(HW, "[oxgrf] device type: IQX%d\n", model);
    } else if(model == 8800) {
        oxgrf->BoardType = IQX8800;
        shift = 2;
        LOG_I(HW, "[oxgrf] device type: IQX%d\n", model);
    ...
    if(oxgrf->BoardType == IQX8800
            || oxgrf->BoardType == IQX8400
            || oxgrf->BoardType == Y750) {
        switch ((int)openair0_cfg->sample_rate) {
        case 245760000:
            openair0_cfg[0].samples_per_packet  = 61440;
            openair0_cfg[0].tx_sample_advance   = 111;
        ...
        case 61440000:
            openair0_cfg[0].samples_per_packet  = 30720;
            openair0_cfg[0].tx_sample_advance   = 172;
            openair0_cfg[0].tx_bw               = 40e6;
            openair0_cfg[0].rx_bw               = 40e6;
            break;
        case 30720000:
            openair0_cfg[0].samples_per_packet  = 15360;
            openair0_cfg[0].tx_sample_advance   = 169;
            openair0_cfg[0].tx_bw               = 20e6;
            openair0_cfg[0].rx_bw               = 20e6;
            break;
        case 15360000:
    ...
    case external:
        ...
        oxgrf_set_ref_clock (oxgrf->dev, 0, OXGRF_EXTERNAL_REFERENCE);
        oxgrf_set_pps_select (oxgrf->dev, 0, OXGRF_PPS_EXTERNAL_EN);
        break;
    case gpsdo:
    ...
    case internal:
    default:
        oxgrf_set_ref_clock (oxgrf->dev, 0, OXGRF_INTERNAL_REFERENCE);
        oxgrf_set_pps_select (oxgrf->dev, 0, OXGRF_PPS_INTERNAL_EN);
        //oxgrf_set_vco_select (oxgrf->dev, 0, OXGRF_AUXDAC1);
        LOG_I(HW, "[oxgrf] clock_source: internal\n");
        break;
    }
    oxgrf_set_auxdac1 (oxgrf->dev, 0, auxdac1);
    if (pa_status) {
        oxgrf_set_duplex_select (oxgrf->dev, 0, OXGRF_FDD);
        ...
        oxgrf_set_duplex_select (oxgrf->dev, 0, 0);
        oxgrf_set_trxsw_fpga_enable(oxgrf->dev, 0, 1);
        oxgrf_set_rx_ant_enable (oxgrf->dev, 0, 0);
    } else {
        oxgrf_set_duplex_select (oxgrf->dev, 0, OXGRF_FDD);
