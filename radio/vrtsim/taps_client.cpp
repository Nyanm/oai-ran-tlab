...
struct taps_client_t {
  ...
  taps_client_t(const char *socket_path) : should_run_(true)
  {
  ...
  channel_desc_t *get_model(int id)
  {
    ...
    AssertFatal(num_rx * num_tx <= MAX_TX_RX_ANTENNAS * MAX_TX_RX_ANTENNAS,
                "Antenna count exceeds maximum: %ux%u\n",
                num_rx,
                num_tx);
...
extern "C" void *taps_client_connect(const char *socket_path)
{
  return new taps_client_t(socket_path);
}
