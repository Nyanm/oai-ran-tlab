                                      ...
                                      int flags)
{
  ...
  for (int i = 0; i < MAX_FD_RFSIMU; i++) {
    ...
    if (b->conn_sock >= 0) {
      ...
      fullwrite(b->conn_sock, tx_beams.data(), sizeof(int) * num_beams, t);
                             ...
                             int flags)
{
  ...
  while (nsamps > 0) {
    ...
    std::vector<int> tx_beams = get_beams(&t->beam_ctrl->tx, timestamp, nsamps, &nsamps_beam_map);
...
static bool add_client(rfsimulator_state_t *t)
{
  ...
  int beam_ids[t->tx_num_channels];
  for (int i = 0; i < t->tx_num_channels; i++)
    beam_ids[i] = 0;
...
static void process_recv_header(rfsimulator_state_t *t, buffer_t *b, bool first_time)
{
  ...
  size_t beam_payload_size = b->th.nbAnt * sizeof(int);
  ...
  while (!packets_copy.empty()) {
    ...
    for (uint aatx = 0; aatx <  pkt->header.nbAnt; aatx++) {
      ...
      for (int aatx = 0; aatx < num_aatx; aatx++) {
        c16_t *buffer = (c16_t *)&pkt->payload[sizeof(int) * pkt->header.nbAnt];
