...
static void process_gains(char *str, rfsim_beam_ctrl_t *beam_ctrl)
{
  ...
  while (token && num_gains < MAX_BEAMS) {
    gain_array[num_gains++] = atof(token);
    token = strtok(NULL, ",");
  }
  ...
  if (num_gains != 0) {
    ...
    char beam_matrix_buf[4096];
    int offset = 0;
    offset += snprintf(beam_matrix_buf + offset, sizeof(beam_matrix_buf) - offset, "beam matrix initialized to:\n");
    for (int i = 0; i < num_gains; i++) {
      offset += snprintf(beam_matrix_buf + offset, sizeof(beam_matrix_buf) - offset, "  [");
      for (int j = 0; j < num_gains; j++) {
        offset += snprintf(beam_matrix_buf + offset,
                           sizeof(beam_matrix_buf) - offset,
                           "%s%.3f",
                           (j > 0 ? ", " : ""),
                           beam_ctrl->beam_gains[i][j]);
      }
      offset += snprintf(beam_matrix_buf + offset, sizeof(beam_matrix_buf) - offset, "]\n");
    }
    LOG_A(HW, "%s", beam_matrix_buf);
  }
}
...
/**
 * @brief Combines transmitted beams from received packets into a single buffer.
 *
 ...
 * @param rx_beam_id The receiver beam ID.
 * @param samples Pointer to the output buffer where combined samples will be stored.
 */
void combine_tx_beams(rfsimulator_state_t *t,
                      std::queue<rfsim_packet_t *> &received_packets,
    ...
    if (ptr->conn_sock != -1 && !ptr->received_packets.empty()) {
      ...
      if (ptr->channel_model != NULL) { // apply a channel model
        ...
        if (is_first_beam && is_first_peer && (ptr->nbAnt == 1 || nbAnt == 1)) {
          // optimization: if only one antenna either on TX or RX side, we can directly write to output buffer. Disable MIMO simulation below
          combine_tx_beams(t, ptr->received_packets, timestamp, 1, nsamps, rx_beam_id, samples);
        } else {
          ...
          combine_tx_beams(t, ptr->received_packets, timestamp, ptr->nbAnt, nsamps, rx_beam_id, input);
          for (int aarx = 0; aarx < nbAnt; aarx++) {
                                  ...
                                  int num_beams)
{
  ...
  static int calls = 0;
  if (calls++ % 10000 == 0) {
    LOG_I(HW, "Rfsimulator: velocity %.2f Msps, realtime requirements %.2f Msps\n", average / 1e6, t->sample_rate / 1e6);
  }
