...
int main(int argc, char **argv)
{
  ...
  while ((c = getopt(argc, argv, "--:O:a:b:d:e:f:g:hi:k:m:n:o:p:q:s:t:v:w:x:y:z:A:F:GH:I:L:M:N:PR:S:T:U:X:Z:")) != -1) {
  ...
  */
  ...
  int uid = 0;
  for (int UE_id = 0; UE_id < number_of_UEs; UE_id++) {
    secondaryCellGroup[UE_id] = get_default_secondaryCellGroup(scc, UE_Capability_nr, 0, 1, &conf, uid);
  ...
  for (int UE_id = 0; UE_id < number_of_UEs; UE_id++) {
    ...
    nr_rrc_mac_config_req_mib(UE_id, 0, mib->message.choice.mib, false, false);
  ...
  for (SNR = snr0; SNR < snr1 && !stop; SNR += .2) {
    ...
    for (int UE_id = 0; UE_id < number_of_UEs; UE_id++) {
      ...
      effTP[UE_id] += effRate[UE_id] / (8 * TBS);
