...
int main(int argc, char **argv)
{
  ...
  if (input_fd==NULL) {
    ...
    for (i=0; i<frame_parms->Lmax; i++) {
      if((SSB_positions >> i) & 0x01) {
        ...
        for (aa=0; aa<gNB->frame_parms.nb_antennas_tx; aa++)
          memset(gNB->common_vars.tx_grid_info[aa].dataF, 0, frame_parms->samples_per_slot_wCP * sizeof(int32_t));
    ...
    LOG_M("txsigF0.m", "txsF0", gNB->common_vars.tx_grid_info[0].dataF, frame_parms->samples_per_slot_wCP, 1, 1);
    if (gNB->frame_parms.nb_antennas_tx > 1)
      LOG_M("txsigF1.m", "txsF1", gNB->common_vars.tx_grid_info[1].dataF, frame_parms->samples_per_slot_wCP, 1, 1);
