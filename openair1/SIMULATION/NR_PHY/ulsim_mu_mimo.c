...
int main(int argc, char *argv[])
{
  ...
  for (SNR = snr0; SNR <= snr1 && !stop; SNR += snr_step) {
    ...
    for (trial = 0; trial < n_trials && !stop; trial++) {
      ...
      UE->ul_harq_processes[0].round = 0;
      ...
      for (int i = 0; i < (TBS / 8); i++) {
        UE->ul_harq_processes[0].payload_AB[i] = i & 0xff;
      }
      ...
      txlev_sum =
          compute_tx_energy_level(UE->common_vars.txData, UE->frame_parms.nb_antennas_tx, symbol_offset, symbol_length, n_trials);

      double sigma = compute_noise_variance(txlev_sum, gNB->frame_parms.ofdm_symbol_size, nb_rb, precod_nbr_layers, SNR, n_trials);
      ...
      for (i = 0; i < available_bits; i++) {
        const uint8_t current_bit = (UE->ul_harq_processes[0].f[i / 8] >> (i & 7)) & 1;
        if (((current_bit == 0) && (pusch_vars->llr[i] <= 0)) || ((current_bit == 1) && (pusch_vars->llr[i] >= 0))) {
          errors_scrambling++;
        }
      }

      if (n_trials == 1 && errors_scrambling > 0) {
        printf(
            "\x1B[31m"
      ...
      for (i = 0; i < TBS; i++) {
        ...
        uint8_t test_input_bit = (UE->ul_harq_processes[0].payload_AB[i / 8] & (1 << (i & 7))) >> (i & 7);
    ...
    printf("*****************************************\n");
    printf("SNR %f: n_errors (%d", SNR, n_errors);
    printf(") (negative CRC), errors_scrambling (%u/%u", errors_scrambling, available_bits);
    printf(")\n");
    printf("\n");

    blerStats = (double)n_errors / n_trials;
    effTP = effRate/(double)TBS * (double)100;
    printf("SNR %f: Channel BLER (%e", SNR, blerStats);

    printf(") Eff Rate %.4f bits/slot, Eff Throughput %.2f, TBS %u bits/slot\n", effRate, effTP, TBS);
