...
int main(int argc, char *argv[])
{
  ...
  uint8_t snr1set = 0;
  int slot = 8, frame = 1;
  float **s_interleaved[MAX_NUM_UE], **r_re, **r_im;
  float **r_re_temp, **r_im_temp;
  ...
  int tx_offset;
  double txlev_sum[MAX_NUM_UE] = {0};
  int start_rb = 0;
  int print_perf = 0;
  cpuf = get_cpu_freq_GHz();
  double effRate[MAX_NUM_UE];
  double effTP[MAX_NUM_UE];
  ...
  void *h_tx_sig_pinned[MAX_NUM_UE] = {NULL};

  while ((c = getopt(argc, argv, "--:O:a:b:c:d:e:g:h:i:jk:l:m:n:o::p:q:r:s:t:u:v:w:y:z:A:C:F:G:H:I:M:N:PR:S:T:U:L:ZW:E:X:Y:"))
         != -1) {
    ...
    switch (c) {
      ...
      case 'e':
        N_UE = atoi(optarg);
        AssertFatal(N_UE > 0 && N_UE <= MAX_NUM_UE,
                    "Number of UEs %d is greater than Maximum supported UEs %d\n",
                    N_UE,
                    MAX_NUM_UE);
        break;
      ...
      case 'h':
        ...
        printf("-W Number of layer for PUSCH per UE\n");
        exit(-1);
        break;
    }
  }
  ...
  r_re_temp = malloc(n_rx * sizeof(float *));
  r_im_temp = malloc(n_rx * sizeof(float *));
  ...
  secondaryCellGroup->spCellConfig->reconfigurationWithSync = get_reconfiguration_with_sync(n_rnti, uid, scc, frame);
  ...
  PHY_VARS_NR_UE *UE[N_UE];
  NR_UE_MAC_INST_t *UE_mac[N_UE];
  PHY_vars_UE_g = malloc(sizeof(PHY_VARS_NR_UE **));
  PHY_vars_UE_g[0] = malloc(N_UE * sizeof(PHY_VARS_NR_UE *));
  ...
  for (int u = 0; u < N_UE; u++) {
    ...
    h_tx_sig_pinned[u] = malloc(padded_tx_alloc_bytes);
    if (h_tx_sig_pinned[u] == NULL) {
      printf("Error: Failed to allocate host buffer for CPU path\n");
      exit(-1);
    }

    // Configure UE
    UE[u] = calloc(1, sizeof(PHY_VARS_NR_UE));
    PHY_vars_UE_g[0][u] = UE[u];
    ...
    UE[u]->no_phase_pre_comp = false;
    UE_mac[u]->if_module = nr_ue_if_module_init(u);
    nr_ue_phy_config_request(&UE_mac[u]->phy_config);

    s_interleaved[u] = malloc(n_tx * sizeof(float *));
    for (int aatx = 0; aatx < n_tx; aatx++) {
      s_interleaved[u][aatx] = calloc(1, frame_length_complex_samples * 2 * sizeof(float));
    }
  }
  ...
  for (int aarx = 0; aarx < n_rx; aarx++) {
    ...
    r_re_temp[aarx] = calloc(1, frame_length_complex_samples * sizeof(float));
    r_im_temp[aarx] = calloc(1, frame_length_complex_samples * sizeof(float));
  }
  ...
  uint8_t ulsch_input_buffer[N_UE][TBS / 8];
  for (int u = 0; u < N_UE; u++) {
    for (i = 0; i < TBS / 8; i++) {
  ...
  unsigned int available_bits = nr_get_G(nb_rb, nb_symb_sch, nb_re_dmrs, number_dmrs_symbols, 0, mod_order, precod_nbr_layers);
  ...
  fapi_nr_ul_config_request_t ul_config[MAX_NUM_UE];
  nr_phy_data_tx_t phy_data[MAX_NUM_UE];
  UE_nr_rxtx_proc_t UE_proc[MAX_NUM_UE];
  memset(ul_config, 0, sizeof(ul_config));
  memset(phy_data, 0, sizeof(phy_data));
  memset(UE_proc, 0, sizeof(UE_proc));

  //---------------
  int ret = 1;
  for (SNR = snr0; SNR <= snr1 && !stop; SNR += snr_step) {
    varArray_t *table_rx = initVarArray(1000, sizeof(double));
    memset(effRate, 0, sizeof(effRate));
    ...
    uint32_t errors_scrambling[MAX_NUM_UE] = {0};
    int n_errors[MAX_NUM_UE] = {0};
    ...
    for (trial = 0; trial < n_trials && !stop; trial++) {
      ...
      for (int u = 0; u < N_UE; u++) {
        ...
        pusch_pdu->pusch_data.new_data_indicator = true;
      ...
      if (N_UE > 0) {
        ...
        for (int g = 0; g < UL_tti_req->n_group; g++) {
          nfapi_nr_ul_tti_request_number_of_groups_t *group = &UL_tti_req->groups_list[g];
          group->n_ue = N_UE;
          for (int u = 0; u < N_UE; u++) {
        ...
        AssertFatal(false, "Number of UEs must be atleast 1\n");
      }
      ...
      for (int u = 0; u < N_UE; u++) {
        for (int aarx = 0; aarx < n_rx; aarx++) {
          memset(r_re_temp[aarx], 0, frame_length_complex_samples * sizeof(float));
          memset(r_im_temp[aarx], 0, frame_length_complex_samples * sizeof(float));
        }
        UE[u]->ul_harq_processes[0].round = 0;
        UE_proc[u].nr_slot_tx = slot;
        UE_proc[u].frame_tx = frame;
        UE_proc[u].gNB_id = 0;

        // --------- setting parameters for UE --------
        nr_scheduled_response_t scheduled_response = {.ul_config = &ul_config[u], .phy_data = (void *)&phy_data[u]};
        ...
        for (int i = 0; i < (TBS / 8); i++) {
          UE[u]->ul_harq_processes[0].payload_AB[i] = ulsch_input_buffer[u][i];
        }
        ...
        txlev_sum[u] = compute_tx_energy_level(UE[u]->common_vars.txData,
        ...
        float *h_tx_ptr = (float *)h_tx_sig_pinned[u];
        memset(h_tx_ptr, 0, n_tx * padded_slot_length * 2 * sizeof(float));

        for (int j = 0; j < n_tx; j++) {
          tx_sig_for_cpu[j] = h_tx_ptr + (j * padded_slot_length + padding_len) * 2;
          memcpy(tx_sig_for_cpu[j], s_interleaved[u][j], slot_length * 2 * sizeof(float));
        }

        start_meas(&channel_stats);
        multipath_channel_float(UE2gNB[u], tx_sig_for_cpu, r_re_temp, r_im_temp, slot_length, 0, (n_trials == 1) ? 1 : 0);
        stop_meas(&channel_stats);

        for (int aarx = 0; aarx < n_rx; aarx++) {
          for (i = 0; i < slot_length; i++) {
            r_re[aarx][i] += r_re_temp[aarx][i];
            r_im[aarx][i] += r_im_temp[aarx][i];
          }
        }
      }
      ...
      double avg_txlev = 0;
      for (int u = 0; u < N_UE; u++) {
        avg_txlev += txlev_sum[u];
      }
      avg_txlev /= N_UE;
      double sigma = compute_noise_variance(avg_txlev, gNB->frame_parms.ofdm_symbol_size, nb_rb, precod_nbr_layers, SNR, n_trials);
      ...
      //----------------------------------------------------------
      NR_gNB_ULSCH_t *ulsch_gNB = NULL;
      NR_gNB_PUSCH *pusch_vars = NULL;
      for (int u = 0; u < N_UE; u++) {
        ulsch_gNB = &gNB->ulsch[u];
        pusch_vars = &gNB->pusch_vars[u];

        if ((ulsch_gNB->last_iteration_cnt >= ulsch_gNB->max_ldpc_iterations) || ul_proc_error == 1) {
          n_errors[u]++;
        } else {
          effRate[u] += ((double)TBS);
        }

        for (i = 0; i < available_bits; i++) {
          const uint8_t current_bit = (UE[u]->ul_harq_processes[0].f[i / 8] >> (i & 7)) & 1;
          if (((current_bit == 0) && (pusch_vars->llr[i] <= 0)) || ((current_bit == 1) && (pusch_vars->llr[i] >= 0))) {
            errors_scrambling[u]++;
          }
        }
        if (n_trials == 1 && errors_scrambling[u] > 0) {
          printf(
              "\x1B[31m"
        ...
        for (i = 0; i < TBS; i++) {
          ...
          uint8_t test_input_bit = (UE[u]->ul_harq_processes[0].payload_AB[i / 8] & (1 << (i & 7))) >> (i & 7);
    ...
    printf("*****************************************\n");
    printf("SNR %f (MU-MIMO %d UEs)\n", SNR, N_UE);
    int ues_passed = 0;
    for (int u = 0; u < N_UE; u++) {
      effTP[u] = (effRate[u] / (double)n_trials) / (double)TBS * 100.0;

      printf("  UE %d: BLER %e (%d errors), Scrambling Errs (%u/%u)\n",
             u,
             (double)n_errors[u] / n_trials,
             n_errors[u],
             errors_scrambling[u] / n_trials,
             available_bits);
      printf("  UE %d: Eff Rate %.4f bits/slot, Eff Throughput %.2f %%, TBS %u bits/slot\n",
             u,
