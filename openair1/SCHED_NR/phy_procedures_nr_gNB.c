...
void nr_save_ul_tti_req(PHY_VARS_gNB *gNB, nfapi_nr_ul_tti_request_t *UL_tti_req, struct nr_grid_slot *nrg)
{
  DevAssert(gNB != NULL);
  DevAssert(UL_tti_req != NULL);

  nrg->frame = UL_tti_req->SFN;
  nrg->slot = UL_tti_req->Slot;
  ...
  for (int i = 0; i < UL_tti_req->n_pdus; i++) {
    ...
    fill_rx_grid_info(gNB->RU_list[0], nrg->frame, nrg->slot, &UL_tti_req->pdus_list[i], nrg);
  }
}
