...
void nr_common_signal_procedures(PHY_VARS_gNB *gNB, int frame, int slot, const nfapi_nr_dl_tti_ssb_pdu *ssb_pdu)
{
  ...
  nr_generate_pss(&txdataF[ant_port][txdataF_offset], gNB->TX_AMP, ssb_start_symbol, cfg, fp);
  nr_generate_sss(&txdataF[ant_port][txdataF_offset], gNB->TX_AMP, ssb_start_symbol, cfg, fp);
  ...
  nr_generate_pbch_dmrs(nr_gold_pbch(fp->Lmax, gNB->gNB_config.cell_config.phy_cell_id.value, hf, ssb_index & 7),
                        &txdataF[ant_port][txdataF_offset],
                        gNB->TX_AMP,
                        ssb_start_symbol,
                        cfg,
                        fp);
  ...
  nr_generate_pbch(gNB, ssb_pdu, &txdataF[ant_port][txdataF_offset], ssb_start_symbol, n_hf, frame, cfg, fp);
}
...
void clear_slot_beamid(PHY_VARS_gNB *gNB, int slot)
{
  ...
  if (gNB->common_vars.beam_id)
    for (int i = 0; i < slot_sz; i++) {
      memset(gNB->common_vars.beam_id[slot * slot_sz + i], 0, gNB->num_ant_ports * sizeof(**gNB->common_vars.beam_id));
    }
}
                           ...
                           int do_meas)
{
  ...
  for (int aa = 0; aa < gNB->num_ant_ports; aa++) {
    memset(&gNB->common_vars.txdataF[aa][txdataF_offset], 0, fp->samples_per_slot_wCP * sizeof(**gNB->common_vars.txdataF));
  }
                          ...
                          int16_t *timing_advance_offset_nsec)
{
  ...
  c16_t **rxdataF = gNB->common_vars.rxdataF + srs->ant_port_start;
...
int phy_procedures_gNB_uespec_RX(PHY_VARS_gNB *gNB, int frame_rx, int slot_rx, NR_UL_IND_t *UL_INFO)
{
  ...
  for (int i = 0; i < gNB->max_nb_pucch; i++) {
    ...
    if (!(pucch && pucch->active && pucch->frame == frame_rx && pucch->slot == slot_rx))
      continue;
    c16_t **rxdataF = gNB->common_vars.rxdataF + pucch->ant_port;
    pucch_decode_done = 1;
  ...
  for (int ULSCH_id = 0; ULSCH_id < gNB->max_nb_pusch; ULSCH_id++) {
    ...
    nr_rx_pusch_tp(gNB, ULSCH_id, frame_rx, slot_rx, ulsch->harq_pid, ulsch->ant_port_start);
  ...
  if (pucch_decode_done || pusch_decode_done) {
    T(T_GNB_PHY_PUCCH_PUSCH_IQ,
      T_INT(frame_rx),
      T_INT(slot_rx),
      T_BUFFER(&gNB->common_vars.rxdataF[0][0], nb_symb * ofdm_symbol_size * 4));
  }
