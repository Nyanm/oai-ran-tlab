...
/* Define vector lane types for HIP in BOTH passes (safe); codegen benefits only in device pass. */
  ...
  typedef union { uint32_t u; gpu_u16x2  v; } gpu_u32_u16x2;
  typedef union { uint32_t u; gpu_s16x2  v; } gpu_u32_s16x2;
...
GPUHD uint32_t gpu_vcmpeq2(uint32_t a, uint32_t b) {
#if defined(__CUDA_ARCH__)
  return __vcmpeq2(a, b);
#elif defined(__HIP_DEVICE_COMPILE__)
  gpu_u32_s16x2 A; A.u = a;
  gpu_u32_s16x2 B; B.u = b;
  gpu_u32_s16x2 R;
  R.v = (A.v == B.v); 
  return R.u;
#else
  ...
  gpu_u32_lanes R;
  R.s16[0] = (A.s16[0] == B.s16[0]) ? 0xFFFF : 0x0000;
  R.s16[1] = (A.s16[1] == B.s16[1]) ? 0xFFFF : 0x0000;
  return R.u;
#endif
}

GPUHD uint32_t gpu_vmaxu2(uint32_t a, uint32_t b) {
#if defined(__CUDA_ARCH__)
  return __vmaxu2(a, b);
#elif defined(__HIP_DEVICE_COMPILE__)
  gpu_u32_u16x2 A; A.u = a;
  gpu_u32_u16x2 B; B.u = b;
  gpu_u32_u16x2 R;
  R.v = (A.v > B.v) ? A.v : B.v;
  return R.u;
#else
  ...
  gpu_u32_lanes B; B.u = b;
  gpu_u32_lanes R;
  R.u16[0] = (A.u16[0] > B.u16[0]) ? A.u16[0] : B.u16[0];
  R.u16[1] = (A.u16[1] > B.u16[1]) ? A.u16[1] : B.u16[1];
  return R.u;
#endif
}

GPUHD uint32_t gpu_vminu2(uint32_t a, uint32_t b) {
#if defined(__CUDA_ARCH__)
  return __vminu2(a, b);
#elif defined(__HIP_DEVICE_COMPILE__)
  gpu_u32_u16x2 A; A.u = a;
...
GPUHD uint32_t gpu_vabs2(uint32_t a) {
#if defined(__CUDA_ARCH__)
  return __vabs2(a);
#elif defined(__HIP_DEVICE_COMPILE__)
  gpu_u32_s16x2 A; A.u = a;
  gpu_u32_s16x2 R;
  R.v = (A.v < (int16_t)0) ? -A.v : A.v;
  return R.u;
#else
  gpu_u32_lanes A; A.u = a;
  gpu_u32_lanes R;
  R.s16[0] = (A.s16[0] < 0) ? -A.s16[0] : A.s16[0];
  R.s16[1] = (A.s16[1] < 0) ? -A.s16[1] : A.s16[1];
  return R.u;
#endif
}

GPUHD uint32_t gpu_vcmplts2(uint32_t a, uint32_t b) {
#if defined(__CUDA_ARCH__)
  return __vcmplts2(a, b);
#elif defined(__HIP_DEVICE_COMPILE__)
...
GPUHD uint32_t gpu_vneg2(uint32_t a) {
#if defined(__CUDA_ARCH__)
  return __vneg2(a);
#elif defined(__HIP_DEVICE_COMPILE__)
  gpu_u32_i16x2 A; A.u = a;
