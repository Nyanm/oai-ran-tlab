...
gpuStream_t decoderStreams[MAX_NUM_DLSCH_SEGMENTS_DL];
gpuStream_t decoderDoneEvents[MAX_NUM_DLSCH_SEGMENTS_DL];
...
#if INT16LLR
extern int16_t *p_llr_dev;
#else
extern int8_t *p_llr_dev;
#endif
extern int8_t *p_out_dev;
...
void LDPCint_rm_init(int max_num_pxsch) {
  ...
  for (int i=0;i<max_num_pxsch;i++) {
    err=gpuMalloc((void **)&harq_d_array[i],MAX_NUM_DLSCH_SEGMENTS_DL*OAI_LDPC_DECODER_MAX_NUM_LLR*sizeof(int16_t));
