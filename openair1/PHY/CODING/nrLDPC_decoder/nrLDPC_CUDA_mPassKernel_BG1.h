...
#if INT16LLR
//----------------------------------↓↓↓ INT16 ↓↓↓---------------------------------------
__device__ __forceinline__ void llrPreProc_Kernel_BG1_int16_Gn_stream(const int16_t *p_llr,
                                                                      int16_t *p_llrProcBuf,
                                                                      int16_t *p_cnProcBuf,
                                                                      uint32_t MsgIdx,
                                                                      uint32_t lane,
                                                                      uint32_t colIdx,
...
{
  {
    ...
    uint16_t bricksLocal[2];
    uint16_t *BricksToBeMoved = bricksLocal;

    p_cnProcBufBit = (uint32_t *)(p_cnProcBuf + d_lut_numCnInCnGroups_BG1_R13[GrpIdx] * NR_LDPC_ZMAX * MsgIdx + lane * 2);

    moveBricks_invget_circ_int16((int16_t *)&p_llr[idxBn * Zc], lane * 2, BricksToBeMoved, Zc, circShift);
  ...
  if (colIdx < startColParity) {
    ...
    int32_t *dst = (int32_t *)(&p_llrProcBuf[idxBn] + lane * 2);
    int32_t *src = (int32_t *)(&p_llr[colIdx * Zc] + lane * 2);
    *dst = *src;
  } else {
    colIdx = colIdx - startColParity;
    if (numBn2CnG1 > 0 && colIdx < numBn2CnG1) {
      int32_t *dst = (int32_t *)(&p_llrProcBuf[colIdx * NR_LDPC_ZMAX] + lane * 2);
      int32_t *src = (int32_t *)(&p_llr[colG1 + colIdx * Zc] + lane * 2);
      *dst = *src;
    }
  }
}

__device__ void llr2bit_Kernel_BG1_int16(uint32_t R,
                                         uint8_t *__restrict__ out,
                                         const int16_t *__restrict__ llrRes,
                                         uint32_t numLLR,
                                         uint32_t Zc)
{
  ...
  if (outColIdx < startColParity) {
    ...
    raw_llrs = *(const int32_t *)(&llrRes[idxBn] + lane * 2);
  } else {
    uint32_t srcParityIdx = outColIdx - startColParity;
    if (numBn2CnG1 > 0 && outColIdx < numBn2CnG1) {
      raw_llrs = *(const int32_t *)(llrRes + srcParityIdx * NR_LDPC_ZMAX + lane * 2);
    } else {
      raw_llrs = 0;
    }
  }

  int16_t *p_val = (int16_t *)&raw_llrs;
  uint16_t my_word = 0;

#pragma unroll
  for (int i = 0; i < 2; i++) {
    uint16_t byte_val = (p_val[i] < 0) ? 1 : 0;
    my_word |= (byte_val << (i * 8));
  }

  *(uint16_t *)(&out[outColIdx * Zc + lane * 2]) = my_word;
}

__device__ void llr2bitPacked_Kernel_BG1_int16(uint32_t R,
                                               uint8_t *__restrict__ out,
                                               const int16_t *__restrict__ llrRes,
                                               uint32_t numLLR,
                                               uint32_t Zc)
...
#pragma unroll
  ...
  uint32_t b0 = __shfl_sync(0xffffffff, my_2_bits, (lane & ~3) + 0);
  uint32_t b1 = __shfl_sync(0xffffffff, my_2_bits, (lane & ~3) + 1);
  uint32_t b2 = __shfl_sync(0xffffffff, my_2_bits, (lane & ~3) + 2);
  uint32_t b3 = __shfl_sync(0xffffffff, my_2_bits, (lane & ~3) + 3);

  if ((lane & 3) == 0) {
    uint8_t packed_byte = (b0 << 6) | (b1 << 4) | (b2 << 2) | b3;

    uint32_t outAddr = outColIdx * (Zc >> 3) + (lane >> 2);
...
{
  {
    ...
    moveBricks_invget_circ_int8((int8_t *)&p_llr[idxBn * Zc], lane * 4, BricksToBeMoved, Zc, circShift);
