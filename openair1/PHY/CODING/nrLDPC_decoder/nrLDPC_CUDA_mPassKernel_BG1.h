...
__device__ __forceinline__ void llrRes2llrOut_Kernel_BG1_int8(uint8_t R, int8_t *llrOut, int8_t *llrRes, uint32_t Zc)
{
  uint32_t tid = blockIdx.x * blockDim.x + threadIdx.x;
  uint32_t colIdx = tid / RowLength;
  ...
  uint32_t startColParity = NR_LDPC_START_COL_PARITY_BG1; // 26
                                 ...
                                 uint32_t Zc)
{
  ...
  // 1. Identify the Output Column ID (0 ~ 67) and Lane ID
  uint32_t outColIdx = tid / RowLength;
  uint32_t lane = tid % RowLength; // Offset within the column (in int32 units)

  // Boundary check: Total 68 columns
  ...
  if (outColIdx < startColParity) {
    ...
    uint32_t idxBn = lut_Addr[outColIdx] + lut_Pos[outColIdx] * Zc;
    ...
    if (numBn2CnG1 > 0 && outColIdx < numBn2CnG1) {
      raw_llrs = *(const int32_t *)(llrRes + srcParityIdx * Zc + lane * 4);
    } else {
      raw_llrs = 0;
    }
  }
                                       ...
                                       uint32_t Zc)
{
  ...
  // 1. Identify Output Column ID and Lane
  uint32_t outColIdx = tid / RowLength;
