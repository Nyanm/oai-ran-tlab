...
int16_t* bnProcBuf_dev;
int16_t* llrRes_dev;
int16_t* llrProcBuf_dev;
...
int8_t* p_out_dev;  //output is always int8_t
...
int cuda_support_init_decoder()
{
  ...
  gpuError_t err;
#if INT16LLR
  err = gpuMalloc((void**)&cnProcBuf_dev, sizeof(int16_t) * MAX_NUM_NR_DLSCH_SEGMENTS_PER_LAYER * 4 * NR_LDPC_SIZE_CN_PROC_BUF);
  AssertFatal(err == gpuSuccess, "CUDA Error (cnProcBuf_dev): %s\n", gpuGetErrorString(err));

  err = gpuMalloc((void**)&bnProcBuf_dev, sizeof(int16_t) * MAX_NUM_NR_DLSCH_SEGMENTS_PER_LAYER * 4 * NR_LDPC_SIZE_BN_PROC_BUF);
  AssertFatal(err == gpuSuccess, "CUDA Error (bnProcBuf_dev): %s\n", gpuGetErrorString(err));

  err = gpuMalloc((void**)&llrRes_dev, sizeof(int16_t) * MAX_NUM_NR_DLSCH_SEGMENTS_PER_LAYER * 4 * NR_LDPC_MAX_NUM_LLR);
  AssertFatal(err == gpuSuccess, "CUDA Error (llrRes_dev): %s\n", gpuGetErrorString(err));

  err = gpuMalloc((void**)&llrProcBuf_dev, sizeof(int16_t) * MAX_NUM_NR_DLSCH_SEGMENTS_PER_LAYER * 4 * NR_LDPC_MAX_NUM_LLR);
  AssertFatal(err == gpuSuccess, "CUDA Error (llrProcBuf_dev): %s\n", gpuGetErrorString(err));

  err = gpuMalloc((void**)&p_llr_dev, sizeof(int16_t) * MAX_NUM_NR_DLSCH_SEGMENTS_PER_LAYER * 4 * NR_LDPC_MAX_NUM_LLR);
  AssertFatal(err == gpuSuccess, "CUDA Error (p_llr_dev): %s\n", gpuGetErrorString(err));
  gpuMemset(p_llr_dev, 0, sizeof(int16_t) * MAX_NUM_NR_DLSCH_SEGMENTS_PER_LAYER * 4 * NR_LDPC_MAX_NUM_LLR);
...
static inline uint32_t nrLDPC_decoder_core_dynamic(
#if INT16LLR
                                                   int16_t* p_llr,
#else
                                                   int8_t* p_llr,
...
void init_decoder_warmup()
{
  ...
  #if INT16LLR
  int16_t* dummy_input_llr = NULL;
  uint32_t max_z = 384;
  uint32_t max_n_segs = STATIC_SEG_SIZE;

  size_t input_size_bytes = 68 * max_z * max_n_segs * sizeof(int16_t);
#else
  ...
  gpuHostAlloc((void**)&dummy_input_llr, input_size_bytes, gpuHostAllocMapped);
  gpuHostAlloc((void**)&dummy_output_bits, output_size_bytes, gpuHostAllocMapped);
...
void init_decoder_gpu_structures()
{
  ...
  for (int i = 0; i < MAX_GRAPH_CACHE_SIZE; i++) {
    if (gpu_graph_cache[i].bridge_ptr == NULL) {
      gpuHostAlloc((void**)&gpu_graph_cache[i].bridge_ptr, sizeof(ldpc_cuda_bridge_t), gpuHostAllocMapped);
  ...
  for (int i = 0; i < 8; i++) {
    if (stream_bridges[i] == NULL) {
      gpuHostAlloc((void**)&stream_bridges[i], sizeof(ldpc_cuda_bridge_t), gpuHostAllocMapped);
...
int32_t LDPCinit_cuda()
{
  ...
  if (!decoder_streamsCreated) {
    for (int s = 0; s < 8; ++s) {
      ...
      gpuEventCreate(&decoderDoneEvents[s]);
    }
...
int32_t LDPCshutdown_cuda()
{
  ...
  for (int s = 0; s < 8; ++s) {
    if (decoder_streamsCreated) {
      gpuEventDestroy(decoderDoneEvents[s]);
...
#if INT16LLR
  int numIter = nrLDPC_decoder_core_dynamic((int16_t*)p_llr, (int8_t*)p_out, n_segments, p_decParams, p_profiler, ab);
#else
