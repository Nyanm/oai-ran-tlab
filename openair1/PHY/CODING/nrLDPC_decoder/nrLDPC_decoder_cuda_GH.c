                                           ...
                                           decode_abort_t* ab)
{
  ...
  for (int CudaStreamIdx = 0; CudaStreamIdx < n_segments; CudaStreamIdx++) {
    int8_t* pp_llr = d_temp_in + CudaStreamIdx * 68 * 384;
    int8_t* pp_out = d_temp_out + CudaStreamIdx * 8448; // use temp_out rather than p_out
    ...
    nrLDPC_decoder_scheduler_BG1_cuda_core(p_lut,
                                           ...
                                           CudaStreamIdx,
                                           decoderDoneEvents,
                                           &d_iter_ptr_array[CudaStreamIdx],
                                           &d_PC_Flag_array[CudaStreamIdx]); // stream index passed in
  }
#else
  for (int s = 0; s < n_segments /*MAX_NUM_DLSCH_SEGMENTS_DL*/; s++) {
    iter_ptr_array[s] = 0;
