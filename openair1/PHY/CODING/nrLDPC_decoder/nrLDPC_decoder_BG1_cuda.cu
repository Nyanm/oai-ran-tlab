                                                     ...
                                                     int *PC_Flag)
{
  ...
  const int8_t *p_bnProcBuf_Grp =
      (const int8_t *)(d_bnProcBuf + segIdx * NR_LDPC_SIZE_BN_PROC_BUF + d_lut_startAddrBnGroups_BG1_R13[BnToAddrIdx - 1]);
  const int8_t *p_bnProcBufRes_Grp =
      (const int8_t *)(d_bnProcBufRes + segIdx * NR_LDPC_SIZE_BN_PROC_BUF + d_lut_startAddrBnGroups_BG1_R13[BnToAddrIdx - 1]);
  const int8_t *p_llrProcBuf_Grp =
      (const int8_t *)(d_llrProcBuf + segIdx * NR_LDPC_MAX_NUM_LLR + d_lut_startAddrBnGroupsLlr_BG1_R13[BnToAddrIdx - 1]);
  const int8_t *p_llrRes_Grp = 
      (const int8_t *)(d_llrRes + segIdx * NR_LDPC_MAX_NUM_LLR + d_lut_startAddrBnGroupsLlr_BG1_R13[BnToAddrIdx - 1]);
...
void nrLDPC_BnToCnPC_BG1_R13_cuda_stream_core(const t_nrLDPC_lut *p_lut,
                                              ...
                                              int8_t CudaStreamIdx)
{
  BnToCnPC_Kernel_BG1_R13_int8_BIG_stream<<<Kdim_R13[CudaStreamIdx].grid, Kdim_R13[CudaStreamIdx].block, 0, streams[CudaStreamIdx]>>>(
      p_lut,
      bnProcBufRes,
      cnProcBuf,
      cnProcBufRes,
                                            ...
                                            int8_t CudaStreamIdx)
{
  ...
  OutPut_Kernel_BG1_R13_int8_BIG_stream<<<Kdim_R13[CudaStreamIdx].grid, Kdim_R13[CudaStreamIdx].block, 0, streams[CudaStreamIdx]>>>(
      p_lut,
      Z,
      iter_ptr,
      numMaxIter,
                                            ...
                                            int8_t CudaStreamIdx)
{
  ...
  cnProcKernel_BG1_R23_int8_BIG_stream<<<Kdim_R23[CudaStreamIdx].grid, Kdim_R23[CudaStreamIdx].block, 0, streams[CudaStreamIdx]>>>(p_lut,
                                                                                         cnProcBuf,
                                                                                         cnProcBufRes,
                                                                                         bnProcBuf,
                                                                                         Z,
                                                     ...
                                                     int *PC_Flag)
{
  ...
  const int8_t *p_bnProcBuf_Grp = (const int8_t *)(d_bnProcBuf + segIdx * NR_LDPC_SIZE_BN_PROC_BUF + d_lut_startAddrBnGroups_BG1_R23[BnToAddrIdx - 1]);
  const int8_t *p_bnProcBufRes_Grp = (const int8_t *)(d_bnProcBufRes + segIdx * NR_LDPC_SIZE_BN_PROC_BUF + d_lut_startAddrBnGroups_BG1_R23[BnToAddrIdx - 1]);
  const int8_t *p_llrProcBuf_Grp = (const int8_t *)(d_llrProcBuf + segIdx * NR_LDPC_MAX_NUM_LLR + d_lut_startAddrBnGroupsLlr_BG1_R23[BnToAddrIdx - 1]);
  const int8_t *p_llrRes_Grp = (const int8_t *)(d_llrRes + segIdx * NR_LDPC_MAX_NUM_LLR + d_lut_startAddrBnGroupsLlr_BG1_R23[BnToAddrIdx - 1]);
                                              ...
                                              int8_t CudaStreamIdx)
{
  ...
  BnToCnPC_Kernel_BG1_R23_int8_BIG_stream<<<Kdim_R23[CudaStreamIdx].grid, Kdim_R23[CudaStreamIdx].block, 0, streams[CudaStreamIdx]>>>(p_lut,
                                                                                            bnProcBufRes,
                                                                                            cnProcBuf,
                                                                                            cnProcBufRes,
                                                                                            bnProcBuf,
...
void nrLDPC_OutPut_BG1_R23_cuda_stream_core(const t_nrLDPC_lut *p_lut,
                                            ...
                                            int8_t CudaStreamIdx)
{
  OutPut_Kernel_BG1_R23_int8_BIG_stream<<<Kdim_R23[CudaStreamIdx].grid, Kdim_R23[CudaStreamIdx].block, 0, streams[CudaStreamIdx]>>>(p_lut,
                                                                                          Z,
                                                                                          iter_ptr,
                                                                                          numMaxIter,
                                                                                          PC_Flag,
