                        ...
                        int16_t layer_llr[dlsch->Nl][llrSize])
{
  switch (dlsch->dlsch_config.qamModOrder) {
    ...
    case 8:
      for(int l=0; l < dlsch[0].Nl; l++)
        nr_256qam_llr(rxdataF_comp[l * nb_antennas_rx],
                      dl_ch_mag,
                      dl_ch_magb,
                      dl_ch_magr,
                      layer_llr[l],
...
int nr_rx_pdsch(PHY_VARS_NR_UE *ue,
                ...
                c16_t rho_dl[][dlsch->Nl * dlsch->Nl][rx_size_symbol])
{
  ...
  // Build c16_t** view into rxdataF_comp at the current symbol
  c16_t *p_rxComp[nl * nbRx];
  for (int l = 0; l < nl; l++)
    for (int a = 0; a < nbRx; a++)
      p_rxComp[l * nbRx + a] = rxdataF_comp[symbol][l * nbRx + a];

  // Per-layer pointers directly into dl_ch_mag[symbol] (avoids intermediate copy)
  c16_t *p_ch_mag[nl];
  c16_t *p_ch_magb[nl];
  c16_t *p_ch_magr[nl];
  for (int l = 0; l < nl; l++) {
    p_ch_mag[l] = dl_ch_mag[symbol][l * nbRx];
    p_ch_magb[l] = dl_ch_magb[symbol][l * nbRx];
    p_ch_magr[l] = dl_ch_magr[symbol][l * nbRx];
  }
  ...
  const bool need_rho = do_ml ? (nl == 2 && dlsch_config->qamModOrder <= 6) : false;
  ...
  int nb_rb_pdsch = freq_alloc->num_rbs;
  {
    ...
    if (first_symbol_flag) {
      ...
      if (nl == 1)
        *log2_maxh = (log2_approx(avgs) / 2) + 1 + log2_approx(nbRx >> 1);
      else
        *log2_maxh = (log2_approx(avgs) >> 1) - 0 + log2_approx(nbRx >> 1);
  ...
  if (nb_re_pdsch) {
    const uint8_t qamModOrder = dlsch_config->qamModOrder;
  ...
  if (symbol == (startSymbIdx + nbSymb - 1)) {
    ...
    const uint8_t qamModOrder = dlsch->dlsch_config.qamModOrder;
    ...
    for (int llr_sym = startSymbIdx; llr_sym < startSymbIdx + nbSymb; llr_sym++) {
      if (nl == 2 && qamModOrder <= 6 && do_ml) {
        ...
        nr_compute_ML_llr(rxdataF_comp[llr_sym][0 * nbRx],
                          rxdataF_comp[llr_sym][1 * nbRx],
                          dl_ch_mag[llr_sym][0 * nbRx],
                          dl_ch_mag[llr_sym][1 * nbRx],
                          layer_llr[llr_sym][0],
                          layer_llr[llr_sym][1],
                          rho_dl[llr_sym][1],
                          rho_dl[llr_sym][nl],
