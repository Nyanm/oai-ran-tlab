...
static void nr_dlsch_channel_compensation(uint32_t rx_size_symbol,
                                          ...
                                          c16_t chFext[][nbRx][rx_size_symbol],
                          ...
                          uint32_t noise_var)
{
  ...
  for (int rtx = 0; rtx < nl; rtx++) {//Output Layers row
    ...
    for (int ctx = 0; ctx < nl; ctx++) { // column multi
      ...
      mult_complex_vectors(inv_H_h_H[ctx][rtx],
                           &rxdataF_comp[symbol][ctx * n_rx][0],
                           outtemp,
                           sizeofArray(outtemp),
                           shift - (fp_flag == 1 ? 1 : 0));
  ...
  for (int rtx = 0; rtx < nl; rtx++)
    nr_element_sign(rxdataF_zforcing[rtx], &rxdataF_comp[symbol][rtx * n_rx][0], nb_rb_0, +1);
...
int nr_rx_pdsch(PHY_VARS_NR_UE *ue,
                ...
                c16_t dl_ch_mag[][dlsch->Nl][rx_size_symbol],
                c16_t dl_ch_magb[][dlsch->Nl][rx_size_symbol],
                c16_t dl_ch_magr[][dlsch->Nl][rx_size_symbol],
                ...
                pdsch_scope_req_t *scope_req)
{
  ...
  */
    if (UEScopeHasTryLock(ue)) {
      ...
      if (UETryLockScopeData(ue, pdschRxdataF_comp, sizeof(c16_t), 1,  total_valid_res, &mt)) {
        ...
        for (int i = startSymbIdx; i < startSymbIdx + nbSymb; i++) {
          ...
          UEscopeCopyUnsafe(ue, pdschRxdataF_comp, &rxdataF_comp[i][0], data_size, offset, i);
          offset += data_size;
        }
  ...
  T(T_UE_PHY_PDSCH_IQ,
    ...
    T_BUFFER(&rxdataF_comp[0], 2 * fp->N_RB_UL * 12 * fp->symbols_per_slot * 2));
#endif
