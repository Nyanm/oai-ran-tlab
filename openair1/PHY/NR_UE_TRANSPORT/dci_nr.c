  ...
  for (int j = 0; j < rel15->number_of_candidates; j++) {
    ...
    for (int k = 0; k < rel15->num_dci_options; k++) {
      ...
      nr_pdcch_unscrambling(&pdcch_e_rx[e_rx_cand_idx],
                            ...
                            tmp_e);
      uint16_t Nid;
      const uint32_t crc = polar_decoder_int16(tmp_e, dci_estimation, (uint16_t*)&Nid, 1, sci_ind ? NR_POLAR_SCI_MESSAGE_TYPE : NR_POLAR_DCI_MESSAGE_TYPE, dci_length, L);
			     ...
			     int sci_indication)
{
  ...
  for (int ss_idx = 0; ss_idx < phy_pdcch_config->nb_search_space; ss_idx++) {
    ...
    int n_rb, cset_start;
    if (sci_indication) get_coreset_rballoc(rel15->coreset.frequency_domain_resource, &n_rb, &cset_start);
    else {
