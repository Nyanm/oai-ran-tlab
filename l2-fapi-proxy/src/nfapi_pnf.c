 ...
 *-------------------------------------------------------------------------------
 * Modified by Russell Ford, Samsung Research America, russell.ford@samsung.com
 */
...
void configure_nr_nfapi_pnf(const char *vnf_ip_addr, int vnf_p5_port, const char *pnf_ip_addr, int pnf_p7_port, int vnf_p7_port, int phy_id) {
  ...
  // pnf_nr.phys[0].id = phy_id + 1;
  pnf_nr.phys[0].udp.enabled = 1;
  pnf_nr.phys[0].udp.rx_port = pnf_p7_port + phy_id;
...
int get_ch_trace_lines(FILE* ch_trace_fp, int num_lines) {
    ...
    while (line_idx < num_lines) {
        ...
        ret = fgets(line, 128, ch_trace_fp);
        if (!ret) {
        ...
        token = strtok(line, ",");
        elem->ue_id = atoi(token);
        token = strtok(NULL, ",");
        elem->trace_idx = atoi(token);
        token = strtok(NULL, ",");
...
void *oai_slot_task(void *context)
{
    ...
    for (int i = 0; i < num_ues; ++i) {
        ...
        ch_info[i].csi[0].sinr = 40;
        ch_info[i].csi[0].rsrp = -70;
    ...
    while (true)
    {
        ...
        // Update channel trace ever 100 frames (~1 sec)
        for (int i = 0; i < num_ues; i++) {
            if (sfn % 100 == 0 && slot == 0) {             
                ch_trace_elem_t* ch_trace = get_queue(&ch_trace_queue[i]);
                if (ch_trace) {
                  ch_info[i].csi[0].sinr = ch_trace->sinr_dB;
                  ch_info[i].csi[0].rsrp = ch_trace->rsrp_dBm;
                  // printf("Updating channel trace for slot %d.%d UE %d: SINR=%.3f\tRSRP=%.3f\n", sfn, slot, i, ch_trace->sinr_dB, ch_trace->rsrp_dBm);
                  free(ch_trace);                      
                } // else use prev. ch_info
            } // else use prev. ch_info
